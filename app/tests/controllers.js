...
controllers.accessTestQuestions = async (req, res) => {
    try {
        ...
        if (testresults && !testresults.isCompleted) {
            ...
            return res.status(200).json({ message: 'Test is on going', test, testSession, questionsAttempted: testresults.answers, isVisited: testresults.isVisited, isReviewed: testresults.isReviewed})
